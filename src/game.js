...
(function () {
  ...
  function drawHud(s) {
    ...
    // 幅は**中身を測ってから**決める（天候・会心の札と同じ作法）。
    // 旧150px幅では「さいこう」(16px＝実測70px)が4px溢れ、176pxの決め打ちに直したが
    // それも余白ちょうどで、文字を1字足せばまた破れる形だった。決め打ちをここでもやめる。
    var colGap = 26, padX = 14;
    ctx.font = "500 16px " + MINCHO;
    var w1 = ctx.measureText("だん").width + 2.0;
    var w2 = ctx.measureText("さいこう").width + 2.0 * 3;
    ctx.font = "800 20px " + ROUND;
    var n1 = ctx.measureText(String(s.step)).width, n2 = ctx.measureText(String(best)).width;
    var c1 = Math.max(w1, n1), c2 = Math.max(w2, n2);
    var boxW = padX + c1 + colGap + c2 + padX, boxH = 50;
    var boxX = faceX - faceR - 10 - boxW, boxY = 14;
    panel(boxX, boxY, boxW, boxH, 8);
    label("だん", boxX + padX, boxY + 20, "left");
    num(s.step, boxX + padX, boxY + 42, 20, C.ink, "left");
    label("さいこう", boxX + padX + c1 + colGap, boxY + 20, "left");
    num(best, boxX + padX + c1 + colGap, boxY + 42, 20, C.inkDim, "left");
  ...
  function drawResult(s) {
    ...
    rankUI.hit = null;
    // 札の下端は cy0+ch=496、「もう一夜」は596。順位はこの間に置く。
    // 囲みの上端 = ry-23 なので、ry は 520 より下でないと札に食い込む。
    var ry = 530;
    // 「更新した」は札の中に既にある（だんも れんも こうしん）。あれは**手元の記録**の話なので、
    // ここでは番付の話であることが分かる言葉にする（同じことを二度言わない）。
    // 順位が取れなかった夜は番付の話自体が無いので、この行も出さない。
    if (rankUI.improved && rankUI.line) {
      ctx.font = "700 16px " + MINCHO; ctx.fillStyle = C.kindei;
      textLS("番付を ぬりかえた", W / 2, 514, 2.0, "center");
      ry = 552;
    }
    if (rankUI.line) {
