...
(function () {
  ...
  var SE_NAMES = ["zan", "kiwami", "fukaku", "whiff", "end", "koto", "koto_high"];
  ...
  function voice(key) {
    ...
    try {
      ...
      voicePlaying = src;
      if (bgmGain) {                                   // 声の間はBGMを下げる
        bgmGain.gain.value = 0.07;
        setTimeout(function () { if (bgmGain && !document.hidden) bgmGain.gain.value = 0.16; },
                   Math.min(src.buffer.duration * 1000 + 250, 8000));
      }
    } catch (e) { }
  }
  // 到達段数から、結果で読む台詞を選ぶ
  ...
  function land() {
    ...
    if (d <= win) {                       // 会心
      ...
      s.maxCombo = Math.max(s.maxCombo, s.combo);
      flash("会心", C.moon);
      // 会心は既存2作と同じ琴のワンショット。連が伸びたら高い方に変える
      se(s.combo >= 3 ? "koto_high" : "koto", 0.9);
    } else {
  ...
  function down(e) {
    ...
    if (s.phase === "title") { reset(); state.phase = "play"; startBGM(); voice("start"); return; }
